...
namespace UserBehavior.Recommenders
{
    public class UserCollaborativeFilterRecommender : IRecommender
    {
        ...
        {
            UserPlaceRatings user = ratings.Users.FirstOrDefault(x => x.UserID == userId);
            List<UserPlaceRatings> neighbors = GetNearestNeighbors(user, neighborCount);

            return GetRating(user, neighbors, articleId);
        }

        private double GetRating(UserPlaceRatings user, List<UserPlaceRatings> neighbors, int articleId)
        {
            int articleIndex = ratings.ArticleIndexToID.IndexOf(articleId);

            var nonZero = user.PlaceRatings.Where(x => x != 0);
            ...
            {
                var nonZeroRatings = neighbors[u].PlaceRatings.Where(x => x != 0);
                double avgRating = nonZeroRatings.Count() > 0 ? nonZeroRatings.Average() : 0.0;

                if (neighbors[u].PlaceRatings[articleIndex] != 0)
                {
                    score += neighbors[u].PlaceRatings[articleIndex] - avgRating;
                    count++;
                }
            }
            if (count > 0)
                ...
                if (user.PlaceRatings[articleIndex] == 0)
                {
                    ...
                    for (int u = 0; u < neighbors.Count; u++)
                    {
                        if (neighbors[u].PlaceRatings[articleIndex] != 0)
                        {
                            ...
                            score += neighbors[u].PlaceRatings[articleIndex] - ((u + 1.0) / 100.0);
                            count++;
                        }
                    }
                    if (count > 0)
        ...
        private List<UserPlaceRatings> GetNearestNeighbors(UserPlaceRatings user, int numUsers)
        {
            List<UserPlaceRatings> neighbors = new List<UserPlaceRatings>();
            ...
            for (int i = 0; i < ratings.Users.Count; i++)
            {
                ...
                {
                    ratings.Users[i].Score = comparer.CompareVectors(ratings.Users[i].PlaceRatings, user.PlaceRatings);
                }
            }
