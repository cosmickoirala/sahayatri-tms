...
namespace TravelManagementSystem.Data.Seeders
{
    public static class IdentityDataSeeder<TIdentityUser, TIdentityRole>
        where TIdentityUser : AppUser, new()
        where TIdentityRole : AppRole, new()
    {
        private const string DefaultAdminRoleName = "Administrator";
        private const string DefaultAdminUserEmail = "admin@sahayatri.com";
        ...
        {
            var user = await userManager.FindByEmailAsync("admin@sahayatri.com");
            if (user == null)
            {
